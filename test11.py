...
for i in range(len(positions) - 1):
	...
	spacer = ''
	if k1 == 'gtggttag':
		spacer = whole_promoter[positions[i][1] + 8:positions[i+1][1]]
		k = positions[i+1][1] - positions[i][1] - 8
		if(k > 0 and k < 26):
			#spacers.append([k,spacer])
			spacers[k].append(spacer)
	if k1 == 'ggtaat':
		spacer = whole_promoter[positions[i][1] + 6:positions[i+1][1]]
		k = positions[i+1][1] - positions[i][1] - 6
		if(k > 0 and k < 26):
			#spacers.append([k,spacer])
			spacers[k].append(spacer)
